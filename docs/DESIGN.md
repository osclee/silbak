...
> **v0.13 changelog:** the SILBAK wordmark's construction — hard unblurred offsets, warm on one side and cool on the other, an ink stroke on the accent half — became a system the whole board is built from, instead of a one-off on the title. Field notes, the rungs, the verdict strip, the ledger and the attempt history are now inked and extruded blocks; the ape names and the panel nameplates carry the lettering's split. Nothing about play changed. Two contrast defects were fixed in passing. `ResultCard`'s headline set no colour and so inherited `--forest` from `body`, ~1.6:1 on `--card` — only the loss variant showed it, since the win variant overrides the colour. And the attempt history's guess number was `--mist`, a dark-panel token, sitting on the `--paper` page ground; it is `--moss` now, the token §6 defines for that position. That second one was fixed twice independently — `4456f15` on master reached it in parallel and set plain `#000` — and `--moss` was chosen by hand when the branches met, on the grounds that `#000` is not in the palette. The same commit's other half, restoring the count badge to `--banana`/`--mist` from a hardcoded `#000`, was identical to this branch's and merged without conflict. See §6, "Relief".
>
...
#### Motion: one wind, not sixty metronomes

The first cut gave every element its own symmetric sine at its own period (7–17s, "deliberately not multiples of each other, so layers drift out of phase"). That was the wrong goal. Out of phase is exactly what a breeze is not: a gust arrives, crosses the scene, and everything it touches answers in turn. Sixty independent oscillators read as a mechanism, and a clunky one, because a symmetric sine has the same shape everywhere and the eye locks onto it. Three concrete defects came with it, all visible on screen:

- **Every element popped on load.** A positive `animation-delay` with the default `fill-mode: none` holds an element at its *un-animated* pose for the delay, then snaps it to the 0% keyframe — which was `rotate(-2.6deg)`, not rest. Staggered across the first five seconds after load, with the sun shafts worst: full opacity for 8s, then a jump to 0.3.
- **Vines didn't move with the leaves they hung from.** They were siblings of the canopy layers, not children, so the canopy translated ±10px while each vine's anchor stayed put and the join slid back and forth.
- **Boughs lifted off their trunks.** Their keyframes added `translateY(±2px)` to the rotation, opening and closing a gap at the branch root every cycle; and the canopy layers *rotated* about top-centre, which see-sawed the two corners of the band up and down in opposite directions — a tilting cut-out, not a ceiling of leaves.

**The model now is a single wind field.** Every primary motion runs on one shared gust profile at one period (`--gust-period`, 16s, set inline by the component so TS and CSS share the number). Elements differ only in amplitude (near, large foliage moves most; the far plane barely stirs) and in *phase*: `gustDelay(u)` returns a negative delay that puts an element at horizontal fraction `u` about `u × 2.6s` behind the gust front, so a gust visibly rolls across the page left to right. The profile is not a sine but a gust — a fast push, a spring-back, a second push, an overshoot upwind on release, a weaker puff, then a lull. Every stop is a turning point, so the zero velocity CSS gives each keyframe boundary is physically right, and segments ease on a true half-sine (`cubic-bezier(0.37, 0, 0.63, 1)`) rather than the sharper stock `ease-in-out`. On top of it, near foliage carries a small secondary *flutter* on a nested element at its own short, irregular period (fronds and boughs), or a length-dependent pendulum swing (vines); two transforms on one element don't add, so the flutter has to live one level down. That secondary layer is what stops the shared period from ever reading as a beat.

**The trees move, and the limbs move with them.** Trunks, both bough planes and the canopy band sit inside one full-page container that carries a shared lean — a subtle skew about the page's bottom edge — so a bough's branch root stays welded to its trunk while the tree leans and its own dip rides on top. One container, not one per block: the blocks are stacked siblings in a fixed z-order anyway, and a single leaning layer is one composite per frame instead of four. Vines now live *inside* the near canopy `<svg>`, drawn before the leaf masses, so they inherit that layer's transform exactly and their tops disappear under the foliage. The canopy shears about its top edge instead of rotating (lowest-hanging leaves move most, the ceiling not at all); boughs rotate purely about the branch root. The understory is anchored at the bottom, where the lean is zero, and needs no wrapper.

**Compositing.** The scanline overlay lost its `mix-blend-mode: multiply`: multiplying by black at 4.5% is arithmetically the same as compositing black at 4.5% (a pixel diff confirmed it, max channel delta 2/255), but the blend mode forced a full-page isolation-and-blend pass every frame anything beneath it moved — the single most expensive thing in the scene under software rendering, for no visible result. A CSS transform animated on an SVG `<g>` is painted on the main thread, rasterising the whole containing SVG's damage rect every frame. Everything large now animates on an HTML element or an outer `<svg>` root: each canopy depth is its own stacked `<svg>` (same viewBox and slice, so coordinates line up as if they were still groups), each bough animates its root (right-side ones are mirrored by an SVG `transform` attribute inside, not a CSS `scaleX(-1)` on the root, so the root's transform stays free and its origin is a plain box edge). Inner `<g>` animations remain only for small things: fronds, vines and flutter.

**Every delay is negative.** The loop is already mid-cycle at first paint and nothing ever starts; motes are somewhere along their rise rather than absent for up to twelve seconds. Motes are also two nested loops now (rise-and-fade on the span, a sideways wobble on its `::before`), which traces a wandering curve where one loop could only draw a straight line from A to B.

All of it is ambient background texture and never feedback; no puzzle state is communicated through motion. `prefers-reduced-motion` is handled by the global `*` rule in `global.css` — with negative delays the loops finish instantly and settle on the rest pose (verified: zero running animations, every transform `none`). One consequence worth knowing: the animation owns `opacity` on the shafts, so a plain `opacity:` rule on them is silently ignored; their range is exposed as `--shaft-lo`/`--shaft-hi` and the 720px rule dims those instead.
